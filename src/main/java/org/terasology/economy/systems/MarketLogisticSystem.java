...
/**
 * This system handles transfer and production events. It will also suppress resource requests if either A. the prior
 * produced resources couldn't be stored (there are items in the internal buffer) B. the resources it consumes were not
 * available
 */
...
public class MarketLogisticSystem extends BaseComponentSystem {
    ...
    private final Logger logger = LoggerFactory.getLogger(MarketLogisticSystem.class);
    ...
    public void postBegin() {
        ...
        if (infiniteStorageEntities.iterator().hasNext()) {
            for (EntityRef storageEntity : infiniteStorageEntities) {
                InfiniteStorageComponent infiniteStorageComponent =
                        storageEntity.getComponent(InfiniteStorageComponent.class);
        ...
        if ((event.getConsumptionResourcePackages() == null || checkResourcesFullyAvailable(event.getConsumptionResourcePackages(), event.getConsumptionStorage()))
            ...
            if (event.getConsumptionResourcePackages() != null) {
                for (Map.Entry<String, Integer> resource : event.getConsumptionResourcePackages().entrySet()) {
                    event.getConsumptionStorage().send(new ResourceDestructionEvent(resource.getKey(),
                            resource.getValue()));
                }
            }
            if (event.getProductionResourcePackages() != null) {
                for (Map.Entry<String, Integer> resource : event.getProductionResourcePackages().entrySet()) {
                    event.getProductionStorage().send(new ResourceCreationEvent(resource.getKey(),
                            resource.getValue()));
                }
            }
        }
    ...
    public void onResourceInfoRequest(ResourceInfoRequestEvent event, EntityRef entityRef) {
        ...
        Map<Component, StorageComponentHandler> storageComponentHandlers =
                storageHandlerLibrary.getHandlerComponentMapForEntity(entityRef);
    ...
    public int processResourceDraw(ResourceDrawEvent event, EntityRef entityRef) {
        Map<Component, StorageComponentHandler> targetStorageComponents =
                storageHandlerLibrary.getHandlerComponentMapForEntity(event.getTarget());
        Map<Component, StorageComponentHandler> originStorageComponents =
                storageHandlerLibrary.getHandlerComponentMapForEntity(entityRef);
        ...
        for (Component component : originStorageComponents.keySet()) {
            availableCapacity += originStorageComponents.get(component).availableResourceCapacity(component,
                    event.getResource());
        }
    ...
    public int processResourceStore(ResourceStoreEvent event, EntityRef entityRef) {
        ...
        int availableCapacity = 0;
        for (Component component : targetStorageComponents.keySet()) {
            availableCapacity += targetStorageComponents.get(component).availableResourceCapacity(component,
                    event.getResource());
        }
        int amountLeft = (availableCapacity < event.getAmount()) ? availableCapacity : event.getAmount();
        for (Component component : originStorageComponents.keySet()) {
    ...
    private boolean checkResourcesFullyAvailable(Map<String, Integer> resources, EntityRef entityRef) {
        ...
        for (Map.Entry<String, Integer> resource : resources.entrySet()) {
            ...
            for (Component component : targetStorageComponents.keySet()) {
                capacityLeft -= targetStorageComponents.get(component).availableResourceAmount(component,
                        resource.getKey());
            }
            if (capacityLeft > 0) {
                return false;
    ...
    private boolean checkCapacityFullyAvailable(Map<String, Integer> resources, EntityRef entityRef) {
        ...
        for (Map.Entry<String, Integer> resource : resources.entrySet()) {
            int capacityLeft = resource.getValue();
            for (Component component : targetStorageComponents.keySet()) {
                capacityLeft -= targetStorageComponents.get(component).availableResourceCapacity(component,
                        resource.getKey());
            }
            if (capacityLeft > 0) {
                return false;
    ...
    private int processCreateResource(ResourceCreationEvent event, EntityRef entityRef) {
        Map<Component, StorageComponentHandler> storageComponents =
                storageHandlerLibrary.getHandlerComponentMapForEntity(entityRef);
